output "resource-ids" {
  ...
  Kafka Cluster ID: ${data.confluent_kafka_cluster.enterprise.id}
  ...
  $ confluent kafka topic produce ${confluent_kafka_topic.orders.topic_name} --environment ${data.confluent_environment.main.id} --cluster ${data.confluent_kafka_cluster.enterprise.id} --api-key "${confluent_api_key.app-producer-kafka-api-key.id}" --api-secret "${nonsensitive(confluent_api_key.app-producer-kafka-api-key.secret)}"
  ...
  $ confluent kafka topic consume ${confluent_kafka_topic.orders.topic_name} --from-beginning --environment ${data.confluent_environment.main.id} --cluster ${data.confluent_kafka_cluster.enterprise.id} --api-key "${confluent_api_key.app-consumer-kafka-api-key.id}" --api-secret "${nonsensitive(confluent_api_key.app-consumer-kafka-api-key.secret)}"
